...
const addOne = (arr) => {
  ...
  arr.forEach(num =>localArray.push( num +1));
...
const addExclamation = (arr) => {
  ...
  arr.forEach( strings => modifiedStrings.push(strings + '!'));
  //`addExclamation` that takes an array of strings
  return modifiedStrings;
  // returns a new array with same items but attaches '!' to the end
...
const allUpperCase = (arr) => {
  let localArray = [];
  arr.forEach( toUpper => localArray.push(toUpper.toUpperCase()));
...
//build new array of strings
  const newArray = [];

  words.forEach( currentValue => newArray.push(callback(currentValue)));
  return newArray;
};
...
const addNumbers = (num, arr, times, callback) => {
  // Solution code here...
  // invoke call back as many times an necessary based on the third argument
...
const createList = (availableItems) => {
  ...
  availableItems.forEach(currentItem => {
    if (currentItem.available === true){
      groceryList.push(currentItem.name);
    }
  });
  return groceryList;
};
