...
const typeNum = (arr) => {
  ...
  const newArray = arr.filter((nums)=> typeof(nums)==='number');
  return newArray;
};
...
const containsAnd = (arr) => {
  ...
  const andArray = arr.filter((word)=> word.includes('and'));
  return andArray;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter(statObject => statObject.baseStat>minBaseStat );
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  // iterate through the array and check if the current items basestate is greater than the integer minBaseStat then we will map through that return object and then for every current item we will ask for the name of that specific item.
  const newArray = arr.filter((statObject)=> statObject.baseStat > minBaseStat).map(currentStat=> currentStat.state.name);
...
const evenOddNumericValues = (arr) => {
  let newArray = [];
  const numsOnly = arr.filter((item)=> typeof(item)==='number');
  numsOnly.map((stringed)=> newArray.push
  return newArray;
};
