...
const mapTwoToThe = (arr) => {
  let newArray = [];
  arr.map((num)=> newArray.push(Math.pow(2,num)));
  return newArray;
};
...
const charCode = (arr) => {
  // Solution code here...
  const newThing = arr.map(letter => letter.charCodeAt());
  return newThing;
};
...
//   // Solution code here...
//   return arr.map((stat)=>{
//     stat.name && stat[total] = stat.effort + stat.baseStat;
//   })
// };
